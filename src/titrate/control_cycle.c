...
/*   Updated: 2024/05/22 11:43:39 by mmosk         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	control_cycle(t_stack *stack, t_dsc start, t_hashlist **result)
{
	t_dsc	current_dsc;
	t_dsc	prev_dsc;
	t_hash	hash;
...
int	control_rifle(t_stack *stack, t_uint depth, t_hashlist **result)
{
	t_uint	size;
	t_hash	hash;
	...
	while (size <= depth)
	{
		if (control_cycle(stack, mk_dsc(size), result))
			return (1);
		size++;
	}
	return (0);
